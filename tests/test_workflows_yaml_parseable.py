...
`llm-cost-optimizer`'s workflows use the `run: |` block-scalar form
today and are YAML-safe, so this lock is the inverse safety net: it
makes the unparseable-shape failure *cannot* land here either. A
...
llm-eval-harness#60, this repo's #56.
"""
